...
create table departments (
	...
	monthly_budget	numeric(8,2) -- first number is precision (# of digits); second @ is scale (# of decimal places)
);
...
-- or you can insert multiple records with a single Insert
insert into departments (name, monthly_budget)
values 	
	('IT', 30000),
...
-- not too many resrictions on the column selector list
...
select *
from products
where expiration_date <'01-APR-2019' -- date strin w/o the "date" keyword; format = DD-MMM-YYYY


select *
from products
where expiration_date < date '2019-04-01' -- date strin w/ the "date" keyword; format YYYY-MM-DD
...
-- eliminate duplicates from result_set (not the table)
...
from employees e; -- retrireves 13 (all) employees
...
-- NOT RECOMMENDED (bad readability)
-- you can reference positions (#)
-- same query as above
select *
from employees
order by 6 desc, 5; -- 6 references the "department_id" column ad 5 references the "monthly_income" column
...
update products 
set abbr_name = substring(name,1,4) -- remember substirng is a scalar function
where id < 4; 
